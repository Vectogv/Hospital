...
export default function ActualizarClinica() {
  ...
  async function cargarClinica() {
    const res = await fetch(`${URL}/clinicas?id=eq.${id}`, { headers: HEADERS })
  ...
  async function actualizar() {
    await fetch(`${URL}/clinicas?id=eq.${id}`, {
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        <div style={styles.header}>
          ...
          <h1 style={styles.title}>Editar Clínica</h1>
        </div>

        <label style={styles.label}>Nombre</label>
        <input style={styles.input} value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Nombre de la clínica" />

        <label style={styles.label}>Dirección</label>
        <input style={styles.input} value={direccion} onChange={(e) => setDireccion(e.target.value)} placeholder="Dirección" />
