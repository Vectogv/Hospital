...
export default function Medicos() {
  ...
  async function cargarDatos() {
    const res = await fetch(`${URL}/medicos`, { headers: HEADERS })
    setMedicos(await res.json())
  }

  async function guardar() {
    if (!nombre.trim()) return
    await fetch(`${URL}/medicos`, {
  ...
  async function eliminar(id) {
    if (!window.confirm("¿Eliminar este medico?")) return
    await fetch(`${URL}/medico?id=eq.${id}`, { method: "DELETE", headers: HEADERS })
    cargarDatos()
  }
  ...
  return (
    <div style={styles.page}>
      <div style={styles.header}>
        ...
        <h1 style={styles.title}>Medicos del cocoro</h1>
      </div>

      <div style={styles.card}>
        <div style={styles.cardTitle}>Registrar nuevo medico</div>
      ...
      <div style={styles.card}>
        <div style={styles.cardTitle}>
          Listado de medicos &nbsp;<span style={styles.badge}>{medicos.length}</span>
        </div>
            ...
            <tbody>
              {medicos.map((p) => (
                ...
                >
                  ...
                  <td style={styles.td}>
                    <button style={styles.btnWarning} onClick={() => navigate("actualizar-paciente/" + p.id)}> Editar</button>
